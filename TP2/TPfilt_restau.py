...
#viewimage(im)
#viewimage(noise(im,10))
#viewimage(rotation(im,45,1/2,ech=0,clip=False))
#viewimage(rotation(im,45,ech=1,clip=True))

#rim=im
#for i in range(0,4):
    #rim=rotation(rim,45,1/2,ech=1,clip=True)
#viewimage(rim)
       
#viewimage(rotation(im,45,clip=False))
    
#var_image(filtre_lineaire(noise(im,10),get_gau_ker(2)), 0,0,1,1 )  
    
#viewimage(filtre_lineaire(noise(im,10),get_gau_ker(2)))
#viewimage(median_filter(noise(im,10),r=3))
#viewimage(filtre_lineaire(im,get_gau_ker(2)))
#viewimage(median_filter(im,r=3))

#x=noise(filtre_lineaire(im,get_gau_ker(2)),1)
#viewimage(filtre_inverse(x,get_gau_ker(2)))
#viewimage(filtre_inverse(filtre_lineaire(im,get_gau_ker(2)),get_gau_ker(2)))

#im=noise(im,5)
#viewimage(wiener(im,get_gau_ker(2),17))

#viewimage(filtre_inverse(im,get_gau_ker(3)))
#viewimage(filtre_lineaire(im,np.array([[0, 0, 1],[1,1,1],[0,0,1]])))
#viewimage(skio.imread(r'C:\Users\jutogashi\Documents\Telecom_2A\IMA\IMA201\images_IMA201_filt\carre_flou.tif'))
